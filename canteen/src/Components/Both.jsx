...
const Both = () => {
  ...
  // Use loginStatus from UserContext for reliable persistence
  const userData = useContext(UserDataContext);
  const {name, setName, email, setEmail, phoneNumber, setPhoneNumber, setStudentCode, password, setPassword,avatar, setAvatar, role, setRole, cart, setCart, orders, setOrders, loginStatus} = userData;
  return (
    <div className='main'>
      {data.map((item) => {
        let addItem = async () => {
          ...
          // Save to backend
          const res = await addToCartAPI(userData.studentCode, cartItem);
          if (res.success) {
            setCart(res.cart);
            console.log("Added to cart in Atlas:", res.cart);
          } else {
            alert(res.message || "Failed to add to cart");
          }
        };
        return (
          <div className="containter border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white max-w-56" key={item.Name}>
